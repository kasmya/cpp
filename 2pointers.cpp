...
public:
    vector<int> sortArrayByParityII(vector<int>& nums) {
        ...
        while (i < n && j < n) {
            // move i forward until we find a wrong element (odd at even index)
            while (i < n && nums[i] % 2 == 0) {
                i += 2;
            }

            // move j forward until we find a wrong element (even at odd index)
